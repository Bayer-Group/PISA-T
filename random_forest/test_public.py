...
def get_public_data(task):
    # Get test data.
    dataframe_test = pd.read_csv(osp.join(publicdir, f"pubchem_{task}.csv")).dropna(axis=0)
    # Remove salts.
    remover = SaltRemover() 
    # Clean test dataframe from invalid molecules.
    clean_smiles = []
    for idx, smile in zip(dataframe_test.index, dataframe_test["Clean_Smiles"].values):
        molecule = Chem.MolFromSmiles(smile)
        if not molecule:
            dataframe_test.drop(idx, inplace=True)
        else:
            try:
                clean = remover.StripMol(molecule)
            except:
                dataframe_test.drop(idx, inplace=True)
                continue
            clean_smiles.append(Chem.MolToSmiles(clean))
    
    # Already tried keep first lets see with no duplicates at all.
    #dataframe_test.drop_duplicates(subset="Clean_Smiles", keep=False, inplace=True)

    X_test = np.array([_compute_morgan(i) for i in tqdm(clean_smiles)], dtype=float)
...
def Main(dtype, task):
    ...
    X_test, y_test = get_public_data(task)
    ...
    # Get training private data.
    train_index = np.load("../data/splits/train_indices.npy", allow_pickle=False)
    X_train, labels_train = fingerprints[train_index], interference_metric.loc[train_index]

    # Load the optimized random forest parameters.
    params = pd.read_csv(f"results/validation/params_{dtype}_{task}.csv", index_col=0)
    # Test on public dataset at different thresholds.
    predictions_dict = {str(i): None for i in thresholds}
    performance_dict = {str(i): None for i in thresholds}
    for thresh in thresholds:
        ...
        performance_dict[str(thresh)] = results

    # Save predictions.
    predictions_df = pd.DataFrame(predictions_dict)
    predictions_df.to_csv(f"results/test_public/{dtype}.csv")

    # Save performances.
    perfromances_df = pd.DataFrame(performance_dict)
    perfromances_df.to_csv(f"results/test_public/{dtype}_performances.csv")
