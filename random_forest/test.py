...
def train_val_split(fingerprints, labels):
    ...
    test_index = np.load("../data/splits/test_indices.npy", allow_pickle=False)
...
def Main(dtype):
    ...
    for thresh in thresholds:
        ...
        # Save trained model.
        with open(f"trained_models/{dtype}_{task}_{thresh}.pkl", "wb") as outfi:
            joblib.dump(model, outfi)

    # Save predictions.
    predictions_df = pd.DataFrame(predictions_dict)
    predictions_df.to_csv(f"results/test/{dtype}.csv")
