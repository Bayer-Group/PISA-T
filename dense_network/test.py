...
def Main(dtype):
    ...
    # Get Test data.
    test_index = np.load("../data/splits/test_indices.npy", allow_pickle=True)
    X_test = fingerprints[test_index]
    labels_test = interference_metric.loc[test_index]
    ...
    predictions_df.to_csv(f"results/test/{dtype}_{task}.csv")
