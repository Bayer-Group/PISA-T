...
"""
Test optimized neural network: public test set.
"""
# Data directory.
datadir = "../data/"
publicdir = "../data/"
...
def get_public_data():
    # Get test data.
    dataframe_test = pd.read_csv(osp.join(publicdir, f"pubchem_testset.csv")).dropna(axis=0)
    # Remove salts.
    remover = SaltRemover() 
...
def Main(dtype):
    ...
    X_test, y_test = get_public_data()
    print("\n Loading data ...")

    smiles = pd.read_feather(osp.join(datadir, "smiles.feather")).Clean_Smiles.values
    ...
    X_train, labels_train, X_val, labels_val = train_val_split(fingerprints, wavelen_signal)
    ...
    for thresh in thresholds:
        ...
        # Make predictions.
        with torch.no_grad():
            predictions = torch.sigmoid(model(torch.tensor(X_test, dtype=torch.float32)))
